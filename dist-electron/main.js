...
function hasUsableLocalSessionDetail(detail) {
    if (!detail.session) {
        return false;
    }
...
async function getPreferredSessionDetail(userId, sessionId) {
...
function mergeSearchRows(...groups) {
...
ipcMain.handle("ask:query", async (_event, payload) => {
    ...
    if (await canUseCloudData()) {
        ...
        const fallbackResult = buildAskMemoraAnswer(payload.question, mergeSearchRows(localPrimaryRows, localTranscriptRows, localOcrRows, primaryRows, transcriptRows, ocrRows));
    ...
    const fallbackResult = buildAskMemoraAnswer(payload.question, mergeSearchRows(localPrimaryRows, localTranscriptRows, localOcrRows));
    if (!isOpenAiConfigured()) {
        return fallbackResult;
    }
    const estimatedTokens = estimateTokensFromText(payload.question) +
