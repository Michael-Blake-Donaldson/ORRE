...
async function runChatCompletion(messages: Array<{ role: "system" | "user"; content: string }>, maxTokens: number) {
...
function renderCitationContext(citations: AskCitationInput[]) {
  return citations
    .slice(0, 10)
    .map((citation, index) => {
...
export async function generateAiAskAnswer(question: string, citations: AskCitationInput[], maxTokens = 520) {
  const context = renderCitationContext(citations);
  const system =
    "You are Memora, a factual assistant. Answer only from provided citations. If evidence is weak, say so clearly. Do not invent facts. Keep response concise and useful.";
  const user = [
    `Question: ${question}`,
    "",
...
export async function generateAiSessionSummary(chunks: SummaryChunkInput[], maxTokens = 650) {
  ...
  const system =
    "You summarize meeting/session evidence. Return JSON only with keys overview, keyPoints, actionItems. Avoid fabrications.";
